...
ask_required() {
    ...
    if [[ "$ASSUME_YES" == "1" || ! -t 0 ]]; then
        fail "Автоматический режим: нужно передать значение заранее. Поле: ${prompt}"
        return 1
    fi
...
ask_secret() {
    ...
    if [[ "$ASSUME_YES" == "1" || ! -t 0 ]]; then
        fail "Автоматический режим: нужно передать секрет через env. Поле: ${prompt}"
        return 1
    fi
...
ask_port() {
    ...
    while true; do
        ...
        if [[ "$ASSUME_YES" == "1" || ! -t 0 ]]; then
            fail "Некорректный порт для автоматического режима: $port"
            return 1
        fi
...
configure_firewall() {
    ...
    if confirm "Настроить порт Remnawave Node в UFW сейчас?" "y"; then
        ...
        panel_ip="$(ask_default "IP/CIDR панели, которой разрешить доступ к порту ноды. Оставь пустым, если неизвестно" "${REMNA_PANEL_IP:-}")"
        if [[ -n "$panel_ip" ]]; then
...
issue_ssl_certificate() {
    ...
    if [[ -z "$domain" ]]; then
        ...
        fail "Некорректный домен в SSL_DOMAIN/DOMAIN: $domain"
        return 1
    fi

    email="${ACME_EMAIL:-${EMAIL:-admin@${domain}}}"
    cert_dir="${CERT_DIR:-/opt/remnawave/nginx}"

    if [[ "$ASSUME_YES" != "1" ]]; then
        email="$(ask_default "Email для acme.sh/Let's Encrypt" "$email")"
        cert_dir="$(ask_default "Куда сохранить privkey.key и fullchain.pem" "$cert_dir")"
    fi
...
show_status() {
    ...
    if command_exists ss; then
        ss -lnt 2>/dev/null | awk -v node_port="$DEFAULT_NODE_PORT" 'NR==1 || $0 ~ ":(22|80|443|" node_port ")[[:space:]]"' || true
    fi
